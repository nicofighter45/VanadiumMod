...
public class BossesManagement {
    ...
    public void addAlreadySpawned(@NotNull List<Integer> list){
        for(Integer id : list){
    ...
    public boolean spawnBoss(@NotNull CustomBossConfig boss, ServerWorld world){
        if(boss.getKey() == 1){
            for(Entity he : boss1){
                if(he.getName().equals(new LiteralTextContent("§6Boss 1")) && he.isAlive()){
                    return false;
                }
            }
            spawn1(boss);
            ...
            for(Entity he : boss2){
                if(he.getName().equals(new LiteralTextContent("§6Boss 2")) && he.isAlive()){
                    return false;
                }
            }
            spawn2(boss);
    ...
    private void spawnEn(@NotNull List<Entity> boss){
        for(Entity hs : boss){
            world.spawnEntity(hs);
        }
    }

    private void spawn1(@NotNull CustomBossConfig boss){
    ...
    private void spawn2(@NotNull CustomBossConfig boss){
